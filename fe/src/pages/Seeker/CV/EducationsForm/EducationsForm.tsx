...
const EducationsForm: any = () => {
  ...
  let serviceStates: any = useSelector((state: any) => state?.cv?.userCV.educationExperience
  );
  ...
  const callServiceExperience = (data: any, isRemove?: any) => {
    ...
    if (!isRemove) {
      //Yeni ekleme
      if (updateData === null) {
        const mutation = [...JSON.parse(userCV.educationExperience), data]

        bodyParameters = {
          ...userCV,
          educationExperience: JSON.stringify(mutation)
        };
      } else {
        //Güncelleme 
        ...
        bodyParameters = {
          ...userCV,
          educationExperience: JSON.stringify(replaced)
        };
      }
    } else {
      //Silme
      bodyParameters = {
        ...userCV,
        educationExperience: JSON.stringify(data)
      };
    }
    ...
    return axios.post(API_URL + loginState?.email, bodyParameters,
      ...
      setAllData(JSON.parse(response.data.cv.educationExperience));
  ...
  return (
    <IonPage>
      <IonContent>
        <IonHeader>
          <Header
            title={serviceStates ? "Güncelle" : "Yeni Oluştur"}
          />
        </IonHeader>
        ...
        {allData?.map((item: any, index: any) => (
          <IonCard key={index}>
            <IonCardHeader>
              <IonCardTitle>{item.schoolName}</IonCardTitle>
        ...
        <IonModal ref={modal} onWillDismiss={(ev) => onWillDismiss(ev)}>
                ...
                <IonList>
                  <IonCard>
                    ...
                    <IonItem>
                      ...
                      <IonInput type="date" value={updateData?.startDate}></IonInput>
                    </IonItem>

                    <IonItem>
                      <IonLabel position="floating">Bitiş Tarihi</IonLabel>
                      <IonInput type="date" value={updateData?.endDate}></IonInput>
                    </IonItem>
                    <IonItem>
