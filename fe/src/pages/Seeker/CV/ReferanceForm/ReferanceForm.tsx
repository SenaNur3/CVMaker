...
const ReferanceForm: any = () => {
  let serviceStates: any = useSelector((state: any) => state?.cv?.userCV.references);
  ...
  const callServiceExperience = (data: any, isRemove?: any) => {
    ...
    if (!isRemove) {
      if (updateData === null) {
        const mutation = [...JSON.parse(userCV.references), data]

        bodyParameters = {
          ...userCV,
          references: JSON.stringify(mutation)
        };
      } else {
        //update 
        ...
        bodyParameters = {
          ...userCV,
          references: JSON.stringify(replaced)
        };
    ...
    return axios.post(API_URL + loginState?.email, bodyParameters,
      ...
      setAllData(JSON.parse(response.data.cv.references));
        ...
        {allData?.map((item: any, index: any) => (
          <IonCard key={index}>
            ...
            <IonCardContent>
              <IonList>
                <IonItem>
                  <IonLabel>Şirket Adı</IonLabel>
                  <IonLabel>{item.companyName}</IonLabel>
                </IonItem>
                <IonItem>
                  <IonLabel>İsim Soyisim</IonLabel>
                  <IonLabel>{item.name}</IonLabel>
                </IonItem>
                <IonItem>
                  <IonLabel>Şehir/ilçe</IonLabel>
